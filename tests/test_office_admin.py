...
from office_admin.models import CANCELLED, COMPLETED, CREATING_EVENT_PDFS, ERROR, GETTING_CALENDAR_EVENTS, PRINT_CALENDAR_EVENTS
from tests.fakes import FakeCalendarWorker, FakeDocumentWorker, PassiveWorker


@pytest.fixture
async def admin_context() -> tuple[OfficeAdmin, FakeCalendarWorker, FakeDocumentWorker, PassiveWorker, PassiveWorker]:
    calendar_worker = FakeCalendarWorker()
    document_worker = FakeDocumentWorker()
    printer_worker = PassiveWorker()
...
@pytest.mark.asyncio
async def test_document_completion_marks_task_completed(admin_context) -> None:
    admin, _, _, _, _ = admin_context
    request_id = admin.submit_print_calendar_events("2026-04-21")
    await asyncio.sleep(0)
    events = [
...
async def test_late_callback_after_completion_is_discarded(admin_context) -> None:
    ...
    await admin.document_failed(request_id, "evt-1", "Too late")
...
async def test_queue_full_raises() -> None:
    calendar_worker = FakeCalendarWorker()
    admin = OfficeAdmin(calendar_worker, FakeDocumentWorker(), PassiveWorker(), PassiveWorker())
    try:
        for _ in range(10):
